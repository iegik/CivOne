    ...
    internal partial class AI : BaseInstance
	{
        ...
        */
		...
		internal void CityProduction(City city)
		{
            ...
            // Pending the setup & use of "continent_policy" [see above] we'll assume a modifier of 1
            int policyModifier = 1;

            // TODO 
            //s16 res_food = city_food_prod - settlersFoodCost * settler_counter - 2 * city_size[city_id]; //city_food_prod is a global var

            // The following counts how many of each unit the civ already-has/already-in-production.
            // It looks like there is a pre-continent limit on the number of units for a civ.
            // I'm going to use SWY's code immediately below for the moment.
            // 1ade:09dd
            // 1ade:0a01
            // 1ade:0ab2
            // 1ade:0b0b
            // 1ade:0b4a : desire for defensive unit in city - ignored for now
            ...
            // Determine the "best" production to make, now that defensive requirements are satisfied
            // 1ade:0baf
            // 1ade:0f57
            int bestUnitDesire = 999; // less is better
            int bestBldgDesire = 999;
            int bestDesire = 999;
            IProduction bestUnitProd = null; // TODO not used for AI?
            IProduction bestBldgProd = null; // TODO not used for AI?
            IProduction bestCommonProd = null;

            // Iterate through available production
            foreach (IProduction tryProd in city.AvailableProduction)
            {
                byte foo = tryProd.ProductionId;
                int currentDesire = 999;

                // TODO this is a crappy set of ids
                if (foo < 26) // normal unit
                {
                    currentDesire = checkUnitDesire(foo, city);
                    if (currentDesire < bestUnitDesire)
                    {
                        bestUnitDesire = currentDesire;
                        bestUnitProd = tryProd;
                    }
                }
                else if (foo < 28 ||   // diplomat or caravan
                         foo > 231 )   // city improvement
                {
                    currentDesire = checkBldgDesire(foo, city);
                    if (currentDesire < bestBldgDesire)
                    {
                        bestBldgDesire = currentDesire;
                        bestBldgProd = tryProd;
                    }
                }
                else                // wonder
                {
                    //checkWonderDesire();
                    continue;  // TODO when does an AI city build wonders?
                }

                if (currentDesire < bestDesire)
                {
                    bestDesire = currentDesire;
                    bestCommonProd = tryProd;
                }
            }

            if (bestDesire == 9999 || bestCommonProd == null)
            {
                // Set random production
                IProduction[] items = city.AvailableProduction.ToArray();
                bestCommonProd = items[Common.Random.Next(items.Length)];
            }

            // TODO 1ade:1268 : if current production is palace check
            city.SetProduction(bestCommonProd);
        ...
        internal int checkUnitDesire(byte unitId, City city)
        {
            int currDesire = 9999;
            UnitType unitT = (UnitType)unitId;
            switch (unitT)
            {
                case UnitType.Carrier:
                    return 9999; // never build carrier
                case UnitType.Settlers:
                    // Can the city afford the food cost to build a settler?
                    int res_food = city.FoodIncome - (city.FoodCosts) - (2 * city.Size);
                    int settleFood = (Player.Government is Governments.Despotism ||
                                      Player.Government is Governments.Anarchy) ? 1 : 2;
                    if (settleFood >= res_food)
                        return 9999;
                    break;
                case UnitType.Militia:
                    if (!(Player.Government is Governments.Despotism ||
                          Player.Government is Governments.Anarchy))
                        return 9999; // never build militia for higher government types
                    break;
                case UnitType.Nuclear:
                    // TODO punting on nukes
                    return 9999;
            ...
            // TODO this is quite stupid. We have to create a Unit instance just to be able to examine characteristics
            IUnit temp = Game.CreateUnit(unitT);

            // 1ade:1054 land check/ defense check
            if (temp.Class == UnitClass.Land)
            {
                currDesire = 2; // TODO + count_on_continent;
            }
            else
            {
                // TODO defence / defence per-continent
            }

            // TODO defence desire

            int granaryFactor = 10; // TODO need better name?
            switch (temp.Role)
            {
                case UnitRole.Settler:
                    // 1ade:113c
                    ...
                    if (city.HasBuilding<Granary>())
                        granaryFactor = 4;
                    // TODO apply leader attitude
                    break;
                case UnitRole.Transport:
                    // 1ade:1113 transport check
                    // check continent policy
                    break;
                case UnitRole.Defense:
                    break;
                default:
                    currDesire = currDesire << 1;
                    // TODO check role vs continentPolicy
                    break;
            }

            // want to build the strongest+fastest units
            granaryFactor *= temp.Price + 2;
            granaryFactor *= currDesire;

            int unitCoolness = (temp.Attack + temp.Defense) * (temp.Move + 1);
            granaryFactor /= unitCoolness;
            currDesire = granaryFactor;

            // 1ade:11a8 sea attack check; air check
            if (temp.Role == UnitRole.SeaAttack)
            {
                // TODO check continent policy
                currDesire *= 4;
            }

            if (temp.Class == UnitClass.Air)
                currDesire = currDesire << 1;

            // 1ade:11d9 unit limit check; continent_policy== settle; peace

            temp = null;
            return currDesire;
        }

        internal int checkBldgDesire(byte bldgId, City city)
        {
            Building bldg;

            if (bldgId > 40)
                bldg = (Building)(255 - bldgId);
            else
            {
                // TODO handle diplomat / caravan
                return 9999;
            }
            ...
            int currDesire = 9999; // less is better
            int bldgPolicy = 0;
            ...
            switch (bldg)
            {
                ...
                case Building.Granary:
                    currDesire = city.ShieldTotal >= 3 ? 4 : 8;
                    break;
                ...
                case Building.MarketPlace:
                    currDesire = -((city.TradeTotal >> 1) - 10); // what's in DX?
                    if (city.IsInDisorder)
                        currDesire = -3;
                    break;
                case Building.Library:
                case Building.Bank:
                    currDesire = -(city.TradeTotal / 3 - 10);
                    break;
                case Building.Courthouse:
                    currDesire = -((city.Corruption << 1) - 12);
                    break;
                case Building.CityWalls:
                    currDesire = city.Size / 2 - 10;
                    if (city.HasBuilding<Palace>())
                        currDesire = 4;
                    break;
                case Building.Aqueduct:
                    int temp = 10 - (res_food >> 1); // what's in DX?
                    if (temp <= city.Size)
                        currDesire = -(city.Size + (res_food >> 1) - 14); // what's in DX?
                    break;
                case Building.Cathedral:
                    if (city.IsInDisorder)
                        currDesire = -2;
                        break;
                case Building.University:
                    currDesire = -((city.TradeTotal >> 2) - 10); // what's in DX?
                    break;
                case Building.MassTransit:
                case Building.SdiDefense:
                    // do not build
                    break;
                case Building.Colosseum:
                    if (city.IsInDisorder)
                        currDesire = -1;
                    break;
                case Building.HydroPlant:
                case Building.Factory:
                    currDesire = -(city.ShieldTotal >> 1) - 13; // TODO whats DX?
                    break;
                case Building.PowerPlant:
                    currDesire = -((city.ShieldTotal / 5) - 12);
                    break;
                case Building.RecyclingCenter:
                case Building.MfgPlant:
                case Building.NuclearPlant:
                    currDesire = - ((city.ShieldTotal >> 3) - 12); // TODO whats DX?
                    break;
                case Building.SSStructural:
                case Building.SSComponent:
                case Building.SSModule:
                    // TODO currently punting
                    break;
            }

            // TODO 1ade:0ed5 some confusion changing desire based on barracks and continent policy and leader attitude
            if (currDesire >= 400)
                currDesire = 0x3fff; // confusing
            // TODO 1ade:0f22 some confusion changing desire based on barracks existing
